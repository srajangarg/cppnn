...
#define DELETE_NULL(x) if(x!=NULL){delete x; x=NULL;}
#define DELETE_VEC_NULL(x) if(x!=NULL){delete[] x; x=NULL;}
...
public:
	...
	void cuda() {
		if(!is_cuda) {
			std::cout << "Moving Tensor numel=" << numel() << " to cuda" << std::endl;
			float* cuda_data = NULL;
			cudaMalloc((void **)&cuda_data, numel() * sizeof(float));
			std::cout << "cuda pointer: " << cuda_data << std::endl;
...
// ACCESS
	...
	void sub(Tensor&second, Tensor&out, float scale=1) {
		...
		if(is_cuda) {
			int num_blocks = ceil(((float)numel())/THREADS_PER_BLOCK);
			tensor_sub<<<num_blocks, THREADS_PER_BLOCK>>>(out.data, data, second.data, numel(), scale);
		}
		else {
			for (int i = 0; i < numel(); ++i)
	...
	int arg_minmax(bool max) {
		...
		if(is_cuda) {
			// std::cout <<  "" __FILE__ ":" << __LINE__ << " freed pointer: " << cpu_data << std::endl;
			delete []cpu_data;
		}

		if(ndims>1)
			std::cerr << "Warning: Trying to find arg_minmax of high-dimensional Tensor (dim=" << ndims << ")" << std::endl;
	...
	void print(std::ostream&s) {
		...
		switch(ndims) {
			case 1: print_data(s,cpu_data,shape[0]); break;
			case 2: print_data(s,cpu_data,shape[0],shape[1]); break;
			case 3: print_data(s,cpu_data,shape[0],shape[1],shape[2]); break;
			case 4: print_data(s,cpu_data,shape[0],shape[1],shape[2],shape[3]); break;
