...
int main()
{
    ...
    save_image("blur-box.jpg", img1, H, W, outF);

    sigma = 5, kH = 13, kW = 13;
    generate_gauss_blur_kernel(kernel, inF, kH, kW, outF, sigma);
    func_conv2d(img, inF, H, W, kernel, outF, kH, kW, img1, kH / 2, kW / 2);
    save_image("blur-gauss.jpg", img1, H, W, outF);
    ...
    save_image("edge.jpg", img2, H, W, outF);

    kH = 3, kW = 3;
    generate_highpass_kernel(kernel, inF, kH, kW, outF);
    func_conv2d(img, inF, H, W, kernel, outF, kH, kW, img1, kH / 2, kW / 2);
    img1.clip_(0, 255);
    save_image("highpass.jpg", img1, H, W, outF);

    kH = 3, kW = 3;
    generate_sharpen_kernel(kernel, inF, kH, kW, outF);
    func_conv2d(img, inF, H, W, kernel, outF, kH, kW, img1, kH / 2, kW / 2);
    img1.clip_(0, 255);
    save_image("sharpen.jpg", img1, H, W, outF);

    kH = 3, kW = 3;
    generate_emboss_kernel(kernel, inF, kH, kW, outF);
    func_conv2d(img, inF, H, W, kernel, outF, kH, kW, img1, kH / 2, kW / 2);
    img1.clip_(0, 255);
    save_image("emboss.jpg", img1, H, W, outF);
